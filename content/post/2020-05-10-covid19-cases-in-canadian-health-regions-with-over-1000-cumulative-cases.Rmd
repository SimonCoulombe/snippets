...
create_graph <- function(data, filter_exp, title, geo,mindate= "20200315"){
  ...
  data  %>%
    ...
    scale_alpha_manual(values = c(1, 0.3), labels= c("average number of cases (last 7 days)", "Daily cases")) + # , guide = "none"
    labs(
      title = title,
...
get_province_graph <- function(.prov){
  ...
  create_graph(z,
             TRUE,
             paste0("Daily Covid-19 cases by health regions in ", .prov),
             health_region)
}
```
...
mtl  <- prout[1:which(prout$Quartier == "Westmount"), ] %>% 
  ...
  mutate(cumulative_cases = if_else(cumulative_cases > lag(cumulative_cases), lag(cumulative_cases), cumulative_cases)) %>% ## cumulative can't be bigger than next day.. if so reduce to next day level.
...
cases <- prep_data(mtl, Quartier)
lastdate  <- max(cases$date_report)
create_graph(cases , 
             total> 200,
             "Évolution du nombre de cas de covid-19 dans les arrondissements de Montréal avec plus de 200 cas", 
             group,
             mindate="20200405") +
  labs(caption = "graph par @coulsim, données compilées par @bouchecl")
...
create_graph(cases , 
             total <= 200,
             "Évolution du nombre de cas de covid-19 dans les arrondissements de Montréal avec moins  de 200 cas", 
             group,
             mindate="20200405") +
  labs(caption = "graph par @coulsim, données compilées par @bouchecl")
