...
create_graph <- function(data, filter_exp, title, geo,mindate= "20200315"){
  ...
  data  %>%
    ...
    scale_alpha_manual(values = c(1, 0.3), labels= c("7-day moving average", "Daily count"), name = NULL) + # , guide = "none"
    labs(
      title = title,
...
create_graph(mtl_cases , 
             total> 200,
             "Daily new Covid-19 cases for Montreal with more than 200 cases", 
             group,
             mindate="20200405") +
...
create_graph(mtl_cases , 
             total <= 200,
             "Daily new Covid-19 cases for Montreal with less than 200 cases", 
             group,
             mindate="20200405") +
  labs(caption = "graph by @coulsim, data compiled by @bouchecl from https://santemontreal.qc.ca/population/coronavirus-covid-19/")
