...
download.file("https://raw.githubusercontent.com/SimonCoulombe/saaqmtq/master/data/code_to_mun.tsv", 
              destfile= "temp_files/code_to_mun.tsv")
}
```
...
accidents16 <- read_csv("temp_files/rapports-accident-2016.csv") %>%
  mutate(NO_ROUTE = as.numeric(NO_ROUTE), SFX_NO_CIVIQ_ACCDN= as.character(SFX_NO_CIVIQ_ACCDN))%>%
   rename(heure_accdn = HR_ACCDN)

accidents15 <- read_csv("temp_files/rapports-accident-2015.csv")%>%
  mutate(NO_ROUTE = as.numeric(NO_ROUTE), SFX_NO_CIVIQ_ACCDN= as.character(SFX_NO_CIVIQ_ACCDN))%>%
   rename(heure_accdn = HR_ACCDN)

accidents14 <- read_csv("temp_files/rapports-accident-2014.csv")%>%
  mutate(NO_ROUTE = as.numeric(NO_ROUTE), SFX_NO_CIVIQ_ACCDN= as.character(SFX_NO_CIVIQ_ACCDN))%>%
   rename(heure_accdn = HR_ACCDN)
accidents13 <- read_csv("temp_files/rapports-accident-2013.csv")%>%
  mutate(NO_ROUTE = as.numeric(NO_ROUTE), SFX_NO_CIVIQ_ACCDN= as.character(SFX_NO_CIVIQ_ACCDN))%>%
   rename(heure_accdn = HR_ACCDN)
accidents12 <- read_csv("temp_files/rapports-accident-2012.csv")%>%
  mutate(NO_ROUTE = as.numeric(NO_ROUTE), SFX_NO_CIVIQ_ACCDN= as.character(SFX_NO_CIVIQ_ACCDN)) %>%
   rename(heure_accdn = HR_ACCDN)
 accidents11 <- read_csv("temp_files/rapports-accident-2011.csv")%>%
   mutate(NO_ROUTE = as.numeric(NO_ROUTE), SFX_NO_CIVIQ_ACCDN= as.character(SFX_NO_CIVIQ_ACCDN))

 code_to_mun <- read_tsv("temp_files/code_to_mun.tsv")
...
save(geocode_this, geo, file= paste0("data/geo_",city,"_",type,".rdata"))
} else {load(paste0("data/geo_",city,"_",type,".rdata"))}
