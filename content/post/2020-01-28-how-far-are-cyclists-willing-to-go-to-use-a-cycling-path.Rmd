...
# comparer la route matché à la matché
```{r}
sf::st_as_sf(data.table::rbindlist( 
  list(
    routed_matched_linestrings[28,] %>% mutate(group = paste0("matched_routed_", group)),
    matched[28,] %>% mutate(group = paste0("matched_", group))
  )
)) %>% mapview()
```

# sortir les distances par type de route pour la matché-routé  

```{r}
zzz <- routed_matched_distances <- 
  routed_matched_linestrings %>%
  add_column(api_results = sss$api_results,
             api_call = sss$api_call) %>%
  mutate(
...
hey j'ai calculé les distances par road class!! : 
```{r}
...
```{r}
zzzz <- zzz %>%
  mutate( distances2 = pmap(list(distances, interval_from, interval_to),
...
zzzz %>%  st_set_geometry(NULL) %>% pull(my_summary) %>% bind_rows() %>%
  ...
  summarise(distance = sum(distance)) %>%
  ungroup() %>%
  mutate(pct = distance / sum(distance)) %>%
  arrange(-distance)
```
