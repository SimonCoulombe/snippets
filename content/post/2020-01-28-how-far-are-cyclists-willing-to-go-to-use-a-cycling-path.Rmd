...
I made a twitter survey a couple weeks before the apocalypse to help me pick my next blog post topic and the crowd overwhelmingly agreed that I should use bike gps data and graphhopper to find out how far cyclists are willing to go to use safer infrastructure.  :)  

This is awesome, because I have been looking for a use for [this open data that contains GPS data for ~ 5000 bike trips in Montreal](http://donnees.ville.montreal.qc.ca/dataset/trajets-individuels-velo-enregistre-mon-resovelo) for a while.  
...
This turned out to be quite an undertaking.  I have hit many snags, and I don't have an answer yet.  Hopefully putting this in the open will help someone and maybe increase my odds of finishing the project.   **This blog post will focus on how I get graphhopper running and how I used it.**
...
It includes the following columns, a linestring geometry depicting the path travelled and a column named "liste_segments_jsonb" giving the IDs of the segments travelled.  (looks like the city did some mapmatching too).. 
```{r}
...
mymap <- trips %>% 
  ...
  mapview(zcol ="id",
          #legend = TRUE,
          #color = c("pink", "red", "orang  e", "grey90", "black"),
          #color = RColorBrewer::brewer.pal(5, "PuBuGn")
...
Since the map matching tool requires GPX files as input, we convert the linestring to multipoint, then export each trip to a gpx file name id_NNN.gps using the `pgirmess` package.
...
```{r, echo = TRUE, eval = F, include = F, results = "hide"}
system(
  paste("cd ~/git/",
...
## Routing tool installation and use 
...
matched_shortest_route2 <- matched_shortest_route_linestrings %>%
  #filter(length_routed_matched >0) %>% # didnt return to origin
  filter(length_routed_matched > 500) %>% # travelled at least 500 meters
  mutate( ratio = length_matched / length_routed_matched,
          distances = future_map(api_results, ~get_distances_from_api_results(.x)),
          road_classes = future_map(api_results, ~ get_road_classes_from_api_results(.x) ),
          road_classes_points = future_map(road_classes, ~ get_each_point_road_class_from_road_classes(.x)),
...
```{r}
matched_routed2 %>%  st_set_geometry(NULL) %>%
  inner_join(matched_shortest_route2 %>% st_set_geometry(NULL) %>% select(group)) %>%
  pull(distance_by_road_class) %>% bind_rows() %>%
  group_by(road_class) %>%
  summarise(distance = sum(distance),
            corrected_distance = sum(corrected_distance)) %>%
  ungroup() %>%
...
# ah shit TODO: identify remove trips that loop  back


# Compare shortest route to map-matched route  and map-matched-routed

```{r}

#%>% ungroup() %>% select(group) %>% inner_join(matched_shortest_route2 %>% st_set_geometry(NULL) %>% select(group))

testtest <- bind_rows_sf(
  list(
    matched  %>% select(group) %>% mutate(type = "matched"),
    matched_routed  %>%  select(group) %>%  mutate(type = "matched_routed"),
    matched_shortest_route2 %>% select(group) %>% mutate(type = "shortest_route")
  )
)
```
...
```{r}
testtest %>% filter(group == "1049") %>% mapview(zcol = "type")
testtest %>% filter(group == "1000") %>% mapview(zcol = "type")
testtest %>% filter(group == "1004") %>% mapview(zcol = "type", alpha = 0.3)
testtest %>% filter(group == "10044") %>% mapview(zcol = "type", alpha = 0.3)
testtest %>% filter(group == "2010") %>% mapview(zcol = "type", alpha = 0.3)
testtest %>% filter(group == "2500") %>% mapview(zcol = "type", alpha = 0.3)
