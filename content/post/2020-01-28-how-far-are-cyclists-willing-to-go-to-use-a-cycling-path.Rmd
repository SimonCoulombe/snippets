...
# install map matching graphhopper
instructions in readme here: 
https://github.com/graphhopper/map-matching
```{r}
mycopy_command <- paste0("cp ", here::here("content/post/data/downloads/quebec-latest.osm.pbf"), " ~/git/map-matching")
```
...
```{r}
system('cd ~/git/ && git clone https://github.com/graphhopper/map-matching.git --branch "recent_core" && cd map-matching && mvn package -DskipTests && java -jar matching-web/target/graphhopper-map-matching-web-1.0-SNAPSHOT.jar import quebec-latest.osm.pbf --vehicle="bike"
')
...
http://localhost:8989/route?point=45.381271%2C-75.809054&point=45.380759%2C-75.82253
```{r}
gaa <- matched$geometry[[1]] %>% st_coordinates %>% as_tibble() %>% mutate(texte = paste0("point=", Y, "%2C", X))  %>% pull(texte)
paste(gaa, collapse="&")
```
