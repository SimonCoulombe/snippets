...
### GAM unweighted  (no spline)


here's how I define a logistic regression in mgcv::gam().  note the REML method is passed to set_engine because this option is specific to the mgcv package.  

the result should be identical to the GLM..
...
note1: for some reason, GAMs  need us  to specify the formula twice, once in add_model() and another time in add_formula.

note2:  I need to add `parametric=TRUE` to broom::tidy()  to get the mgcv parameters (https://broom.tidymodels.org/reference/tidy.gam.html)
```{r}

gam_wf <- workflow() %>%
  add_model(gen_additive_mod_mgcv_spec, formula = my_formula) %>%  # need to add formula twice, and  in add_formula
  add_formula(my_formula)
...
gam_wf_fit %>% tidy(parametric = TRUE)
```

let's model this outside tidymodels to make sure it works... 

```{r}
mgcv::gam(formula = my_formula, data = my_train, family = stats::binomial(link = "logit"), method="REML") %>% 
...
gam_wf_weighted_fit <-  gam_wf%>% 
  add_case_weights(exposure_weight) %>% 
  fit(data = my_train )


gam_wf_weighted_fit %>% tidy(parametric = TRUE)
```
same as the weight GLM.. assume this worked.   

## GAM unweighted (with splines)    



here's how I define a logistic regression in mgcv::gam().  note the REML method is passed to set_engine because this option is specific to the mgcv package.  

ok this one is interesting I need to add the spline in the formula in add_model(my_spline_formula), but not in add_formula(my_formula).
...
#gam_spline_wf %>% tidy(parametric = FALSE)
```
same result if I do it directly.

```{r}
mgcv::gam(formula = my_spline_formula, data = my_train, family = stats::binomial(link = "logit"), method="REML") %>% 
  broom::tidy(parametric= TRUE)
```
