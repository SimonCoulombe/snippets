...
Things I would like to figure out eventually:
  ...
  * create `poisson` and `gamma` regressions (should be easy, but I'm out of time).  
  * figure out why my average prediction for the logistic xgboost is so off.  maybe it wasnt done learning? 
  * figure out a way to give a maximum/minimum value to the tune_grid for variables.  I don't want it to test a learning rate of 0.000000000000001.
...
The total dollar value of claims is  `claimcst0`, which we will rename to `dollar_loss`.  We will divide it by the exposure to obtain `annual_loss`.
...
The `annual_loss` variable is what we want to model.  In insurance, it is typically modelled directly using a `tweedie`regression, or indirectly by multiplying the output of two models, one predicting  the frequency of claims  (poisson regression)  and the other predicting the severity of claims (gamma regression).
...
mydb %>% count(policy_id) %>% count(n) %>% gt(caption="most (fake) policy_id have only 1 record in the dataset, but many have more")
```


# grouped and stratified  train/test split and rsamples     

We want all the records for a given `policy_id` to end up in either train or test.  
We also want to try to have roughly the same annual loss in the train and test.  

To do this, we use  `recipes::group_initial_split()`: 
...
before diving into the cool stuff like evaluating the model on multiple resamples and tuning parameters, I'll just make sure I can reproduce the training inside and outside `tidymodels`  
...
yes!
```{r}
logistic_glm_direct_fit <- glm(my_logistic_reg_formula,    family = "binomial", data = my_train) 
...
what if we want weights?   apparently we can use `add_case_weights()` and refer to the `exposure_weight` column of type `importance_weight` we created earlier:
...
```{r}
weighted_logistic_glm_direct_fit <- glm(my_logistic_reg_formula,    family = "binomial", data = my_train, weights = exposure)
...
* Note 2: for some reason, GAMs  need us  to specify the formula twice, once in add_model() and another time in add_formula.  Interesting:   I need to add the spline in the formula in add_model(my_logistic_reg_formula_with_splines), but not in add_formula(my_logistic_reg_formula).
...
Let's start with specifying the hyperparameters.  Tuning will have to wait for now.  
...
Can I train the exact same thing using xgboost?
...
pred_tidy <- predict(logistic_xgb_wf_fit, new_data = my_test, type = "prob")   %>% select(.pred_1)
pred_direct <- predict(direct_xgb_fit, newdata = xgtest)
z <- pred_tidy %>% rename(pred_tidy = .pred_1) %>%  add_column(pred_direct)
...
Alright, that was fun!  Now that I feel I know how to train a model in tidymodels, let's try to define all the models at once using a preprocessor, a formula in this case, and a list of model specifications.  Then we apply fit_resamples() to fit all workflows to all resamples.  

We could have used a recipe as the preprocessor.  We'll try that in another part later.    fit_resamples() is used when we don't do any tuning.  Tuning will be in the next part.



note 1 :  update_workflow_model() **exists** https://github.com/tidymodels/workflowsets/issues/64
 is used to add the formula to the GAM model inside a workflow_set.  

note2 : we're could have used multiple different formulas to test different model specifications.   we'll try that in a part later.  
...
collect_predictions(logistic_tuneable_xgb_wf_fit) %>%
  ...
  autoplot()
```
ROC curve is upside down when using .pred_1. ... guess I'd need to replace the reference level somewhere.. for today I'll just use .pred_0:



```{r}
collect_predictions(logistic_tuneable_xgb_wf_fit) %>%
  roc_curve(truth = has_claim_fct, .pred_0) %>%
