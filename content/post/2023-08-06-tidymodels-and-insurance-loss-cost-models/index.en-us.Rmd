...
What if we want weights?   It doesnt really make sense in this logistic example, but will in the future for tweedie/poisson/gamma.  Apparently we can use `add_case_weights()` and refer to the `exposure_weight` column of type `importance_weight` we created earlier:
...
```
here's the average prediction for all worflow on train:

```{r}
out_of_sample_preds %>% group_by(wflow_id, .config) %>% summarise(unweighted_average = mean(.pred_1))
```

just making sure.. is my weighted average pred equal to my weighted average "has_claim" ?  yup
```{r}
my_train %>% add_column(
  out_of_sample_preds %>%
  ...
  summarise(weighted_average_pred = sum(exposure * .pred_1)/sum(exposure),
