...
* use case weights for all models ,
* use `embed::step_lencode_glm()` to replace high-cardinality variables with embedded values,  
* create `logistic` and  `tweedie` regressions using tidymodels and check that they fit what I would get by fitting the models outside of tidymodels .  
* create models using `GLM`, `GAM`, `xgboost` and `lightgbm`.   Turns out GLM and xgboost are easy.  GAM are harder (you need to pass a formula.. but how do you do it when you don't know what dummies step_other() will generate).  lightgbm 
* use workflow sets to apply the same recipe/formula to a bunch of model specifications and pick the best one using cross validation.   bonus: tune some hyperparameters at the same time.  

Things I would like to do later:   

  * create `poisson` and `gamma` regressions.     
  * figure out a way to add GAMs to workflow_set() when using a recipe that creates new variables (like a step_dummy() after a step_other())).  
...
note2 : we're could have used multiple different formulas to test different model specifications.   
...
# TODO: Logistic Regression with recipe  
...
my_recipe_with_imputation <- recipe(my_train ) %>%
  update_role(all_of(labels(terms(my_has_claim_formula))), new_role = "predictor") %>% # assign the role of predictor to right-side terms of my formula
