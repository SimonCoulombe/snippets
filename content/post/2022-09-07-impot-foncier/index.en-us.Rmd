...
pumf_qc_bin <- pumf_qc %>% 
  ...
  mutate(taux_taxation_sur_patrimoine_bin = cut(taux_taxation_sur_patrimoine, c(-Inf,0, 0.0025, 0.0050, 0.0075, 0.01, 0.0125, 0.0150, 0.0175, 0.02, 0.03,0.04,0.05, Inf), right = FALSE)) %>%
  mutate(
    ...
    ,
    levels = c("0.00 à 0.25%","0.25 à 0.50%", "0.50 à 0.75%",
...
sommes_par_bin %>% 
  ...
  tab_header(
    ...
    subtitle = "Bien que le taux moyen effectif observé chez le groupe le plus pauvre (0 à 50 000$) soit de seulement 0.472% car plusieurs d'entre eux ne possèdent pas d'immobilier, plusieurs (3.1%) d'entre eux font face à un taux supérieur à 5%."
