...
binned_pumf_qc <- pumf_qc %>% 
  ...
  summarise(n = n(), sum_PWEIGHT = sum(PWEIGHT),  valeur_fonciere = sum(valeur_fonciere), PWNETWPT = sum(PWNETWPT)) %>%
  mutate(
    taxes_foncieres = 0.01 * valeur_fonciere,
    taux_taxation_sur_patrimoine_du_bin =  if_else(PWNETWPT >0,taxes_foncieres / PWNETWPT,0)
    ) 
...
binned_pumf_qc %>%
  ...
  ggplot(aes(x= avoir_net_bin, y = taux_taxation_sur_patrimoine_du_bin)) + 
  geom_col()+ 
  scale_y_continuous(labels = scales::label_percent(),
                     #limits = c(0, 0.01)
                     ) +
...
Mais attention !  Ces moyennes cachent une grande variabilité des taux effectifs.  On observe chez certains pauvres (avoir net inférieur à 50 000$) un taux de taxation sur l'avoir net supérieur à 5%.   Cela arrive lorsque leur valeur foncière est plus de 5x plus élevé que leur avoir net (par exemple, ils ont achet une maison avec une mise de fond de moins de 20% et cela représentait tout leur argent).
...
pumf_qc_bin %>% 
  ...
  tab_header(
    ...
    subtitle = "Bien que le taux moyen chez le groupe le plus pauvre (0 à 50 000$) soit plus faible, plusieurs (3.1%) d'entre eux font face à un taux effectif supérieur à 5%"
