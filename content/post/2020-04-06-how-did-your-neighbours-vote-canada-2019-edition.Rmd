...
poll_results <- read_rds("poll_results.rds")
poll_pop_shp <- read_rds("poll_pop_shp.rds")
...
```{r out.width='100%', warning=FALSE, message=FALSE, echo=FALSE, include=TRUE}
...
poll_shp <- read_rds("poll_shp.rds")
...
poll_pop_shp <- poll_pop_shp %>% 
  ...
  st_cast("POLYGON")
write_rds(poll_pop_shp ,"poll_pop_shp.rds")
poll_pop_shp <- read_rds("poll_pop_shp.rds")
...
write_rds(poll_results, "poll_results.rds")
```
