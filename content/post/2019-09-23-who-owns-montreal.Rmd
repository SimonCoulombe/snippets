...
* The owner and value of each lot had to be scraped from city's [EvalWeb website](https://servicesenligne2.ville.montreal.qc.ca/sel/evalweb/index) individually.  This involves breaking a (thankfully weak) captcha.     
...
mapdata_mtl_everyone <- read_rds("data/mapdata_mtl_everyone.rds")
mapdata_mtl_non_gouv <- read_rds("data/mapdata_mtl_non_gouv.rds")
```


```{r, echo=F, include = T,   out.width = '15in', out.height='12in'}

mapdata_mtl_everyone %>%
  mapview(zcol= "rang_valeur_surface_id", 
          layer.name = "Top 8 des propriétaires selon la valeur totale<br> 
           (rang global - valeur - superficie - nom principal)",
...
mapdata_mtl_non_gouv %>% 
  mapview(zcol= "rang_valeur_surface_id", 
          layer.name = "Top 8 des propriétaires non-gouvernementaux selon la valeur totale<br> 
           (rang global - valeur - superficie - nom principal)",
          col.region = RColorBrewer::brewer.pal(8, "Accent"))
