...
if (switch_generate_interim_data){
  ...
  random_grid_results <- list_of_param_sets %>% 
    mutate(booster = map(data, function(X){
      ...
      function_return <- list(Score = cv$evaluation_log[, min(test_poisson_nloglik_mean)], # l'itération qui a la plus haute moyenne de nloglik sur toutes les cv folds
                              Pred = 0)
...
} else {random_grid_results <- read_rds( here::here("content/post/data/interim/random_grid_results.rds"))}
```
...
obj.fun  <- smoof::makeSingleObjectiveFunction(
  ...
  fn =   function(x){
    ...
    cv$evaluation_log[, min(test_poisson_nloglik_mean)]
  },
...
hardcoded %>% 
  ...
  select(type,loglikelihood, max_depth, colsample_bytree, subsample,min_child_weight,eta, gamma) %>% knitr::kable()
```
...
```{r}
#Bayesian Optimization wants "a function to be maximized".. so we will take the negative of the negative log likelihood.. lol

xgb_cv_bayes_times_minus_1 <- 
  function(max_depth=4, 
    ...
    list(Score = -1 * cv$evaluation_log[, min(test_poisson_nloglik_mean)],
         Pred = 0)
  }
```
...
```{r bayes_scratch, collapse = FALSE}
...
} else {bayesian_results <- read_rds( here::here("content/post/data/interim/bayesian_results.rds"))}
```



```{r}
knitr::kable(bayesian_results$History)
...
```{r bayes_continued_bayesian, collapse= FALSE}
...
The table below shows the negative-loglikelihood (lower is better) and hyperparameters for the hardcoded `simon_param` model, and the best models found by random search, mlrMBO and rBayesianOptimisation.  
...
hardcoded <- random_grid_results[1,] %>%
  ...
  mutate(nloglikelihood= random_grid_results[1,"Score"] %>% as.numeric())
best_random <- random_grid_results %>% 
  filter(Score == min(Score)) %>% pull(data) %>% .[[1]] %>% 
  mutate(type = "random_grid") %>%
  mutate(nloglikelihood = min(random_grid_results$Score))
...
best_rBayesianOptimization <- 
  ...
  mutate(nloglikelihood = -1 * max(bayesian_results_continued_from_bayesian$History$Value))
hardcoded %>% 
  bind_rows(best_random) %>% 
  bind_rows(best_mlrMBO) %>%
